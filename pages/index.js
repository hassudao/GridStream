...
export default function App() {
  ...
  return (
    <div className={`max-w-md mx-auto min-h-screen pb-20 border-x relative shadow-2xl transition-colors ${darkMode ? 'bg-black text-white border-gray-800' : 'bg-white text-black border-gray-100'}`}>
      ...
      {view === 'home' && (
        <div className="animate-in fade-in">
          ...
          <div className={`divide-y ${darkMode ? 'divide-gray-800' : 'divide-gray-100'}`}>
            {posts.map(post => (
              <PostCard key={post.id} post={post} openProfile={openProfile} getAvatar={getAvatar} onDelete={handleDeletePost} onLike={toggleLike} onSendComment={handleSendComment} onDeleteComment={handleDeleteComment} currentUser={user} darkMode={darkMode} />
            ))}
          </div>
        </div>
      )}
          ...
          <div className="px-4 relative">
            ...
            <div className="flex justify-end py-3">
              {user.id === activeProfileId && (
                <button onClick={() => isEditing ? handleSaveProfile() : setIsEditing(true)} className="border rounded-full px-5 py-1.5 text-xs font-black uppercase">{isEditing ? 'Save' : 'Edit Profile'}</button>
              )}
            </div>
      ...
      <div className="flex gap-3">
        ...
        <div className="flex-grow min-w-0">
          ...
          <div className="flex justify-between mt-4 text-gray-400 max-w-[180px]">
            ...
            <button onClick={() => setShowComments(!showComments)} className={`flex items-center gap-1.5 ${showComments ? 'text-blue-500' : ''}`}>
          ...
          <div className="space-y-4 mb-4 pt-2">
            {post.comment_list.map(comment => (
              <div key={comment.id} className="flex gap-2 text-sm group">
                <img src={getAvatar(comment.profiles?.username, comment.profiles?.avatar_url)} className="w-6 h-6 rounded-full object-cover" />
                <div className="flex-grow">
                  <div className="flex items-center gap-2">
                    <span className="font-black">@{comment.profiles?.username}</span>
                  ...
                  <p className="font-medium opacity-90">{comment.content}</p>
                </div>
              </div>
            ))}
          </div>
          
          <form onSubmit={(e) => { e.preventDefault(); onSendComment(post.id, commentText); setCommentText(''); }} className="flex gap-2">
            <input 
              type="text" 
              placeholder="返信をストリーム..." 
              className={`flex-grow text-xs font-bold p-2 rounded-xl outline-none ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}
              value={commentText}
              onChange={(e) => setCommentText(e.target.value)}
            />
            <button type="submit" disabled={!commentText.trim()} className="text-blue-500 disabled:opacity-30"><Send size={18} /></button>
          </form>
        </div>
      )}
    </div>
    ...
    <div className="animate-in fade-in">
      ...
      <div className="grid grid-cols-3 gap-[2px]">
        {posts.filter(p => p.image_url && p.content.includes(searchQuery)).map(post => (
          <img key={post.id} src={post.image_url} className="aspect-square w-full h-full object-cover cursor-pointer hover:opacity-80" />
        ))}
      </div>
    </div>
  );
...
function AuthScreen({ fetchData, validateProfile }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-8 bg-white font-sans text-black">
      <script src="https://cdn.tailwindcss.com"></script>
      <div className="w-20 h-20 bg-gradient-to-tr from-blue-700 to-cyan-500 rounded-[2rem] flex items-center justify-center shadow-2xl mb-6 animate-pulse"><Zap size={40} color="white" fill="white" /></div>

...
export default function App() {
  ...
  return (
    <div className={`max-w-md mx-auto min-h-screen pb-20 border-x font-sans relative shadow-2xl overflow-x-hidden transition-colors duration-300 ${darkMode ? 'bg-black text-white border-gray-800' : 'bg-white text-black border-gray-100'}`}>
      ...
      {selectedPost && <PostDetailModal post={selectedPost} onClose={() => setSelectedPost(null)} getAvatar={getAvatar} openProfile={openProfile} onDelete={handleDeletePost} onLike={toggleLike} currentUser={user} darkMode={darkMode} refreshPosts={fetchData} />}
      ...
      {view === 'home' && (
        <div className="animate-in fade-in">
          ...
          <div className={`divide-y ${darkMode ? 'divide-gray-800' : 'divide-gray-100'}`}>
            {posts.map(post => (
              <PostCard key={post.id} post={post} openProfile={openProfile} getAvatar={getAvatar} onDelete={handleDeletePost} onLike={toggleLike} currentUser={user} darkMode={darkMode} onOpenDetail={() => setSelectedPost(post)} />
            ))}
          </div>
        </div>
      )}
...
function PostCard({ post, openProfile, getAvatar, onDelete, onLike, currentUser, darkMode, onOpenDetail }) {
      ...
      <div className={`w-full max-w-md rounded-[2.5rem] flex flex-col h-[85vh] overflow-hidden ${darkMode ? 'bg-black border border-gray-800' : 'bg-white'}`}>
        ...
        <div className="flex-grow overflow-y-auto">
          ...
          <div className="p-5 space-y-4 pb-10">
            <h4 className="text-[10px] font-black uppercase tracking-widest text-gray-500 mb-2">Comments ({comments.length})</h4>
            {comments.length === 0 ? (
              <p className="text-gray-400 text-xs italic font-medium">まだコメントはありません。</p>
            ) : (
              comments.map(c => (
                <div key={c.id} className="flex gap-3 animate-in fade-in">
                  <img src={getAvatar(c.profiles?.username, c.profiles?.avatar_url)} className="w-8 h-8 rounded-full object-cover mt-1" />
                  <div className={`flex-grow p-3 rounded-2xl ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
                    <div className="flex justify-between items-center mb-1">
                      <span className="font-black text-xs">@{c.profiles?.username}</span>
                    </div>
        ...
        <form onSubmit={handlePostComment} className={`p-4 border-t flex gap-2 ${darkMode ? 'bg-black border-gray-800' : 'bg-white'}`}>
          <input 
            type="text" 
            placeholder="コメントを入力..." 
            className={`flex-grow p-4 rounded-2xl text-sm outline-none font-medium ${darkMode ? 'bg-gray-900 text-white' : 'bg-gray-50'}`}
            value={commentText}
          ...
          <button 
            ...
            className="bg-blue-600 text-white p-4 rounded-2xl shadow-lg active:scale-95 transition flex items-center justify-center disabled:opacity-50"
          >
            <Send size={18}/>
          </button>
        </form>

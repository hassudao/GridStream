...
import { 
  ...
  Zap, LogOut, Mail, Lock, Settings, AlertCircle, Trash2, MessageSquare, 
  UserCheck, AtSign, CheckCircle2 
} from 'lucide-react';
...
export default function App() {
  ...
  return (
    <div className={`max-w-md mx-auto min-h-screen pb-20 border-x font-sans relative shadow-2xl transition-colors duration-300 ${darkMode ? 'bg-black text-white border-gray-800' : 'bg-white text-black border-gray-100'}`}>
      ...
      {selectedPost && <PostDetailModal post={selectedPost} onClose={() => setSelectedPost(null)} getAvatar={getAvatar} openProfile={openProfile} currentUser={user} darkMode={darkMode} />}
          ...
          <div className="px-4 relative">
            <div className="flex justify-between items-end -mt-14 mb-4">
              ...
              {user.id === activeProfileId && (
                <button onClick={() => setView('home')} className={`border rounded-full px-6 py-2 text-xs font-black uppercase tracking-tighter shadow-sm ${darkMode ? 'border-gray-700 bg-gray-900' : 'border-gray-200 bg-white'}`}>Edit</button>
              )}
            </div>
            <div className="mt-2">
              ...
              <p className="mt-4 text-[15px] font-medium leading-relaxed">{profileInfo.bio || 'GridStream member.'}</p>
              <div className="flex gap-6 mt-6">
                <button onClick={() => setShowFollowList('following')} className="text-sm font-bold"><span className="font-black">{stats.following}</span> <span className="text-gray-400 uppercase text-[10px]">Following</span></button>
                <button onClick={() => setShowFollowList('followers')} className="text-sm font-bold"><span className="font-black">{stats.followers}</span> <span className="text-gray-400 uppercase text-[10px]">Followers</span></button>
              </div>
            </div>
          </div>
          <div className="flex border-b mt-8 sticky top-0 bg-inherit z-10 border-gray-100/10">
            <button onClick={() => setProfileTab('list')} className={`flex-grow py-4 flex justify-center ${profileTab === 'list' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-400'}`}><List size={20}/></button>
            <button onClick={() => setProfileTab('grid')} className={`flex-grow py-4 flex justify-center ${profileTab === 'grid' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-400'}`}><Grid size={20}/></button>
          </div>
          <div className={profileTab === 'grid' ? "grid grid-cols-3 gap-0.5" : "divide-y divide-gray-100/10"}>
      ...
      <nav className={`fixed bottom-0 max-w-md w-full border-t flex justify-around py-4 z-40 backdrop-blur-lg ${darkMode ? 'bg-black/95 border-gray-800 text-gray-600' : 'bg-white/95 border-gray-100 text-gray-300'}`}>
...
// --- Sub-Components (With Direct Tailwind) ---

function PostCard({ post, openProfile, getAvatar, currentUser, darkMode, onOpenDetail }) {
  return (
    <article className="p-4 flex gap-4 transition-colors hover:bg-gray-50/5">
      <img src={getAvatar(post.profiles?.username, post.profiles?.avatar_url)} className="w-12 h-12 rounded-full cursor-pointer object-cover shadow-lg" onClick={() => openProfile(post.profiles.id)} />
      <div className="flex-grow min-w-0">
        ...
        {post.image_url && <img src={post.image_url} onClick={onOpenDetail} className="mt-4 rounded-[2rem] w-full max-h-80 object-cover border border-gray-100/10 cursor-pointer shadow-md" />}
        <div className="flex gap-8 mt-5 text-gray-400 items-center">
          ...
          <button onClick={onOpenDetail} className="flex items-center gap-2 hover:text-blue-500 transition-colors"><MessageSquare size={20} /></button>
...
function SettingsScreen({ onClose, user, darkMode, setDarkMode }) {
  return (
    <div className={`fixed inset-0 z-[110] transition-colors duration-300 ${darkMode ? 'bg-black text-white' : 'bg-white text-black'}`}>
      <header className="p-4 border-b flex items-center gap-4 sticky top-0 z-10 bg-inherit border-gray-100/10">
      ...
      <div className="p-6 space-y-6">
        <section className="space-y-4">
          <h3 className="text-gray-400 text-[10px] font-black uppercase tracking-widest">Appearance</h3>
          <button onClick={() => setDarkMode(!darkMode)} className={`w-full flex justify-between items-center p-5 rounded-[2rem] border transition-all ${darkMode ? 'bg-gray-900 border-gray-800' : 'bg-gray-50 border-gray-100 shadow-sm'}`}>
        ...
        <button onClick={() => supabase.auth.signOut()} className="w-full p-5 rounded-[2rem] bg-red-500/10 text-red-500 font-black uppercase text-xs tracking-widest flex items-center justify-center gap-3">
...
function MessagesList({ allProfiles, user, setDmTarget, getAvatar, openProfile, darkMode }) {
  return (
    <div className="flex flex-col">
      <header className="p-6 border-b font-black text-xl text-center uppercase italic tracking-tighter sticky top-0 z-10 bg-inherit border-gray-100/10">GridStream Chat</header>
      <div className="p-3 space-y-1">
        {allProfiles.filter(p => p.id !== user.id).map(u => (
          <div key={u.id} className="flex items-center gap-5 p-4 rounded-[2rem] cursor-pointer hover:bg-blue-600/5 transition-colors group" onClick={() => setDmTarget(u)}>
            <img src={getAvatar(u.username, u.avatar_url)} className="w-16 h-16 rounded-full object-cover shadow-xl group-hover:rotate-3 transition-transform" />
            <div className="flex-grow border-b border-gray-100/10 pb-4">
              <p className="font-black text-[15px]">{u.display_name}</p>
              <p className="text-[10px] text-blue-500 font-black mt-1 italic uppercase tracking-widest">Start Streaming</p>
            </div>
          </div>
        ))}
      </div>
...
function DMScreen({ target, setDmTarget, currentUser, getAvatar, darkMode }) {
  ...
  return (
    <div className={`fixed inset-0 z-[120] flex flex-col transition-colors duration-300 ${darkMode ? 'bg-black' : 'bg-[#fcfcfc]'}`}>
      ...
      <form onSubmit={send} className="p-6 border-t flex gap-3 bg-inherit border-gray-100/10">
...
function PostDetailModal({ post, onClose, getAvatar, openProfile, currentUser, darkMode }) {
  return (
    <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-2xl flex items-center justify-center p-4">
      <div className={`w-full max-w-md rounded-[3rem] flex flex-col h-[90vh] overflow-hidden shadow-2xl border ${darkMode ? 'bg-black border-gray-800' : 'bg-white border-gray-100'}`}>
        <div className="p-6 border-b flex justify-between items-center bg-inherit border-gray-100/10">
...
function AuthScreen({ fetchData }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-10 bg-white">
      <div className="w-24 h-24 bg-gradient-to-br from-blue-700 to-cyan-400 rounded-[2.5rem] flex items-center justify-center shadow-2xl mb-8 rotate-12 animate-pulse"><Zap size={48} color="white" fill="white" /></div>

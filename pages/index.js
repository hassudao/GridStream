  ...
  return (
    <div className={`max-w-md mx-auto min-h-screen pb-20 border-x font-sans relative shadow-2xl overflow-x-hidden transition-colors duration-300 ${darkMode ? 'bg-black text-white border-gray-800' : 'bg-white text-black border-gray-100'}`}>
        ...
        <div className="animate-in fade-in">
          ...
          <div className={`flex gap-4 p-4 overflow-x-auto no-scrollbar border-b ${darkMode ? 'border-gray-800' : 'border-gray-50'}`}>
            <div className="flex flex-col items-center gap-1 flex-shrink-0">
              <div className="w-16 h-16 rounded-full p-1 border-2 border-blue-500"><img src={getAvatar(myProfile.username, myProfile.avatar_url)} className="w-full h-full rounded-full object-cover" /></div>
            ...
            {allProfiles.filter(p => p.id !== user.id).slice(0, 10).map(p => (
              <div key={p.id} className="flex flex-col items-center gap-1 flex-shrink-0 cursor-pointer" onClick={() => openProfile(p.id)}>
                <div className="w-16 h-16 rounded-full p-1 border-2 border-pink-500"><img src={getAvatar(p.username, p.avatar_url)} className="w-full h-full rounded-full object-cover" /></div>
          ...
          <div className="px-4 relative">
            ...
            <div className="mt-4 space-y-3">
              ...
              <div className="flex flex-wrap gap-4 text-gray-500 text-[10px] font-black uppercase tracking-widest">
                <span className="flex items-center gap-1"><MapPin size={12}/> World</span>
      ...
      <nav className={`fixed bottom-0 max-w-md w-full border-t flex justify-around py-4 z-40 shadow-sm ${darkMode ? 'bg-black/95 border-gray-800 text-gray-600' : 'bg-white/95 border-gray-100 text-gray-300'}`}>
        ...
        <Bookmark className="cursor-pointer hover:text-blue-600" onClick={() => {}} />
    ...
    <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-xl flex flex-col items-center justify-center p-4">
      ...
      <div className={`w-full max-w-md rounded-[2.5rem] overflow-hidden ${darkMode ? 'bg-gray-900' : 'bg-white'}`}>
        ...
        <div className="p-5 max-h-[60vh] overflow-y-auto">
          ...
          <p className="font-medium leading-relaxed mb-6">{post.content}</p>
          <div className="flex justify-around py-4 border-t">
            <button onClick={() => onLike(post.id, post.is_liked)} className={`flex flex-col items-center gap-1 ${post.is_liked ? 'text-red-500' : 'text-gray-400'}`}><Heart size={24} fill={post.is_liked ? "currentColor" : "none"} /><span className="text-[10px] font-black uppercase">Like</span></button>
            <button onClick={() => onBookmark(post.id, post.is_bookmarked)} className={`flex flex-col items-center gap-1 ${post.is_bookmarked ? 'text-blue-500' : 'text-gray-400'}`}>{post.is_bookmarked ? <BookmarkCheck size={24} fill="currentColor" /> : <Bookmark size={24} />}<span className="text-[10px] font-black uppercase">Save</span></button>
          </div>
        </div>
      </div>
    </div>
...
function SettingsScreen({ onClose, user, darkMode, setDarkMode }) {
  ...
  return (
    <div className={`fixed inset-0 z-[100] animate-in slide-in-from-bottom duration-300 ${darkMode ? 'bg-black text-white' : 'bg-white text-black'}`}>
      <header className="p-4 border-b flex items-center gap-4 sticky top-0 z-10"><ChevronLeft onClick={onClose} className="cursor-pointer" /><h2 className="font-black uppercase tracking-tighter">Settings</h2></header>
      <div className="p-4 space-y-8">
        <section><h3 className="text-gray-400 text-[10px] font-black uppercase tracking-widest mb-4">Appearance</h3><button onClick={() => setDarkMode(!darkMode)} className={`w-full flex justify-between items-center p-4 rounded-2xl ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}><div className="flex items-center gap-3">{darkMode ? <Moon size={18} className="text-blue-400"/> : <Sun size={18} className="text-orange-400"/>}<span className="text-sm font-bold">Dark Mode</span></div><div className={`w-10 h-6 rounded-full relative transition-colors ${darkMode ? 'bg-blue-600' : 'bg-gray-300'}`}><div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${darkMode ? 'right-1' : 'left-1'}`} /></div></button></section>
        <section><button onClick={handleLogout} className="w-full p-4 rounded-2xl bg-red-50 text-red-500 font-black uppercase text-xs tracking-widest">Logout from Beta</button></section>
      </div>
    </div>
  );
}
...
function FollowListModal({ type, userId, onClose, openProfile, getAvatar, darkMode }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/60 backdrop-blur-sm flex items-end justify-center">
      <div className={`w-full max-w-md rounded-t-[2.5rem] h-[70vh] flex flex-col ${darkMode ? 'bg-black' : 'bg-white'}`}>
        <div className="p-6 border-b flex justify-between items-center"><h3 className="font-black text-lg uppercase text-blue-600">{type}</h3><X onClick={onClose} className="cursor-pointer" /></div>
        <div className="overflow-y-auto p-4 space-y-2">{list.map(u => (
          <div key={u.id} className="flex items-center gap-4 cursor-pointer p-3 rounded-2xl hover:bg-gray-50/10" onClick={() => { openProfile(u.id); onClose(); }}><img src={getAvatar(u.username, u.avatar_url)} className="w-12 h-12 rounded-full object-cover" /><div className="flex-grow"><p className="font-black text-sm">{u.display_name}</p><p className="text-gray-400 text-xs font-bold">@{u.username}</p></div></div>
        ))}</div>
      </div>
    </div>
  );
...
function MessagesList({ allProfiles, user, setDmTarget, getAvatar, openProfile, darkMode }) {
  return (
    <div className="animate-in fade-in">
      <header className="p-4 border-b font-black text-lg text-center uppercase italic">GridStream Chat</header>
      <div className="p-2">{allProfiles.filter(p => p.id !== user.id).map(u => (
        <div key={u.id} className="flex items-center gap-4 p-4 rounded-2xl cursor-pointer hover:bg-gray-50/10" onClick={() => setDmTarget(u)}><img src={getAvatar(u.username, u.avatar_url)} className="w-14 h-14 rounded-full object-cover" onClick={(e) => { e.stopPropagation(); openProfile(u.id); }} /><div className="flex-grow border-b pb-2"><p className="font-bold text-sm">{u.display_name}</p><p className="text-xs text-blue-500 font-medium mt-1 italic uppercase">Tap to Stream</p></div></div>
      ))}</div>
    </div>
  );
}
...
function DMScreen({ target, setDmTarget, currentUser, getAvatar, darkMode }) {
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex flex-col animate-in slide-in-from-right duration-300 ${darkMode ? 'bg-black' : 'bg-gray-50'}`}>
      <header className={`p-4 flex items-center gap-3 border-b ${darkMode ? 'bg-gray-900 border-gray-800' : 'bg-white'}`}><ChevronLeft onClick={() => setDmTarget(null)} className="cursor-pointer" /><img src={getAvatar(target.username, target.avatar_url)} className="w-10 h-10 rounded-full object-cover" /><div><p className="font-black text-sm">{target.display_name}</p><p className="text-[10px] text-gray-400 font-bold">@{target.username}</p></div></header>
      <div className="flex-grow overflow-y-auto p-4 space-y-4">
        {messages.map(m => (
          <div key={m.id} className={`flex ${m.sender_id === currentUser.id ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[80%] p-4 rounded-[1.5rem] text-sm ${m.sender_id === currentUser.id ? 'bg-blue-600 text-white rounded-tr-none' : (darkMode ? 'bg-gray-800 text-white rounded-tl-none' : 'bg-white text-gray-800 rounded-tl-none shadow-sm')}`}>{m.text}</div></div>
        ))}
        <div ref={scrollRef} />
      </div>
      <form onSubmit={sendMsg} className="p-4 border-t flex gap-2"><input type="text" className={`flex-grow p-4 rounded-2xl text-sm outline-none ${darkMode ? 'bg-gray-900 text-white' : 'bg-white shadow-inner'}`} placeholder="Type message..." value={text} onChange={(e) => setText(e.target.value)} /><button type="submit" className="bg-blue-600 text-white p-4 rounded-2xl shadow-lg"><Send size={18}/></button></form>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-8 bg-white font-sans text-black">
      ...
      <form onSubmit={handleAuth} className="w-full max-w-xs space-y-4">
        ...
        <button type="submit" className="w-full bg-blue-700 text-white font-black py-5 rounded-2xl shadow-xl uppercase tracking-widest text-xs">{isLogin ? 'Login' : 'Join'}</button>
      </form>

...
const PostCard = ({ post, openProfile, getAvatar, onLike, onShare, currentUser, darkMode, onOpenDetail, onDelete }) => (
    ...
    <div className="flex-grow min-w-0">
      <div className="flex justify-between items-start mb-1">
        ...
        <div className="flex items-center gap-2">
          ...
          {currentUser?.id === post.user_id && (
            <button onClick={(e) => { e.stopPropagation(); onDelete(post.id); }} className="text-gray-400 hover:text-red-500 transition">
              <Trash2 size={14}/>
            </button>
          )}
        </div>
      ...
      {post.image_url && (
        <img 
          ...
          className="mt-3 rounded-2xl w-full max-h-80 object-cover border border-gray-100/10 cursor-pointer hover:brightness-95 transition" 
        />
      )}
      <div className="flex justify-between mt-4 text-gray-400 max-w-[200px] items-center">
        ...
        <button onClick={() => onShare(post)} className="hover:text-green-500 transition">
          <Share2 size={18} />
        </button>
      </div>
    </div>
...
const StoryCreator = ({ file, onClose, onPublish, myProfile, getAvatar }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black flex flex-col overflow-hidden touch-none" onMouseMove={handleMove} onTouchMove={handleMove} onMouseUp={() => setIsDragging(false)} onTouchEnd={() => setIsDragging(false)}>
      <img ref={imgRef} src={previewSrc} className="hidden" alt="hidden-preview" />
      ...
      <div id="story-preview-container" className="flex-grow relative flex items-center justify-center bg-gray-900 overflow-hidden">
        {previewSrc && (
          <div className="relative w-full h-full max-w-md flex items-center justify-center">
            <img src={previewSrc} className="w-full h-full object-contain pointer-events-none" alt="story-base" />
            {text && (
              <div 
        ...
        <div className="absolute bottom-0 w-full p-4 flex justify-between items-center bg-gradient-to-t from-black/90 to-transparent pb-8">
          <div className="flex items-center gap-2">
            <div className="p-[2px] rounded-full bg-gradient-to-tr from-yellow-400 to-purple-500">
              <img src={getAvatar(myProfile.username, myProfile.avatar_url)} className="w-8 h-8 rounded-full border-2 border-black" alt="me" />
            </div>
            <span className="text-white text-xs font-bold">あなたのストーリー</span>
          </div>
          <button onClick={handlePublish} className="bg-white text-black rounded-full p-3 px-6 font-bold flex items-center gap-2 shadow-lg active:scale-95 transition">
...
const StoryViewer = ({ stories, initialIndex, onClose, userProfile, getAvatar, currentUserId, onDelete, sendNotification }) => {
  ...
  useEffect(() => {
    ...
    setProgress(0);
    checkLikeStatus(currentStory.id);
    startTimer();
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black flex items-center justify-center">
      ...
      <div 
        className="relative w-full max-w-md h-full sm:h-[90vh] sm:rounded-2xl overflow-hidden bg-gray-900 flex flex-col shadow-2xl" 
        onMouseDown={handlePointerDown} onMouseUp={handlePointerUp} onTouchStart={handlePointerDown} onTouchEnd={handlePointerUp}
      >
        ...
        <div className="absolute top-4 left-0 right-0 z-20 p-3 pt-6 flex justify-between items-center">
          <div className="flex items-center gap-2">
            <img src={getAvatar(userProfile?.username, userProfile?.avatar_url)} className="w-8 h-8 rounded-full border border-white/50" alt="uploader" />
          ...
          <div className="flex items-center gap-2">
            {currentUserId === currentStory.user_id && (
              <button onClick={(e) => { e.stopPropagation(); onDelete(currentStory.id); }} className="text-white/80 hover:text-red-500 p-2"><Trash2 size={20} /></button>
            )}
        ...
        <img src={currentStory.image_url} className="w-full h-full object-contain bg-black animate-in fade-in duration-300" alt="story" />
...
export default function App() {
  ...
  const handleShare = async (post) => {
    const shareData = { title: 'Beta SNS', text: post.content, url: window.location.origin };
    try {
  ...
  const handleDeleteStory = async (storyId) => {
    if(!window.confirm("ストーリーを削除しますか？")) return;
  ...
  return (
    <div className={`max-w-md mx-auto min-h-screen pb-20 border-x font-sans relative shadow-2xl transition-colors duration-300 ${darkMode ? 'bg-black text-white border-gray-800' : 'bg-white text-black border-gray-100'}`}>
        ...
        <div className="animate-in fade-in">
            ...
            <div className="inline-flex flex-col items-center gap-1 cursor-pointer relative shrink-0">
              <div className="relative">
                <div 
                  ...
                  onClick={() => { if (groupedStories[user.id]) setViewingStory({ userId: user.id, index: 0 }); else storyInputRef.current.click(); }}
                >
                  <img src={getAvatar(myProfile.username, myProfile.avatar_url)} className={`w-16 h-16 rounded-full object-cover border-2 ${darkMode ? 'border-black' : 'border-white'}`} alt="me" />
                </div>
               ...
               return (
                 <div key={userId} className="inline-flex flex-col items-center gap-1 cursor-pointer shrink-0" onClick={() => setViewingStory({ userId, index: 0 })}>
                   <div className="p-[2px] rounded-full bg-gradient-to-tr from-yellow-400 via-red-500 to-purple-500">
                     <img src={getAvatar(uProfile.username, uProfile.avatar_url)} className={`w-16 h-16 rounded-full object-cover border-2 ${darkMode ? 'border-black' : 'border-white'}`} alt="u" />
                   </div>
          ...
          <form onSubmit={handlePost} className={`p-4 border-b ${darkMode ? 'border-gray-800' : 'border-gray-100'}`}>
            <div className="flex gap-3">
              <img src={getAvatar(myProfile.username, myProfile.avatar_url)} className="w-10 h-10 rounded-full object-cover cursor-pointer" onClick={() => openProfile(user.id)} alt="me" />
              <textarea 
                className="flex-grow border-none focus:ring-0 text-lg placeholder-gray-400 resize-none h-16 outline-none bg-transparent font-medium" 
            ...
            <div className="flex justify-between items-center pl-12 mt-2">
              <label className="cursor-pointer text-blue-500 p-2 rounded-full transition hover:bg-blue-500/10">
                <ImageIcon size={22}/>
              ...
              <button 
                ...
                className="bg-blue-600 text-white px-6 py-2 rounded-full font-black text-xs uppercase tracking-tighter disabled:opacity-50"
              >
                {uploading ? '...' : 'Stream'}
              </button>
            </div>
      ...
      {view === 'profile' && profileInfo && (
        <ProfileView 
          user={user} activeProfileId={activeProfileId} profileInfo={profileInfo} posts={posts} 
          isEditing={isEditing} setIsEditing={setIsEditing} editData={editData} setEditData={setEditData} 
          ...
          uploading={uploading} avatarInputRef={avatarInputRef} headerInputRef={headerInputRef} 
          getAvatar={getAvatar} openProfile={openProfile} toggleFollow={toggleFollow} stats={stats} 
          setShowFollowList={setShowFollowList} setShowSettings={setShowSettings} darkMode={darkMode} 
          setView={setView} toggleLike={toggleLike} handleShare={handleShare} 
          setSelectedPost={setSelectedPost} onDeletePost={handleDeletePost} 
        />
      )}
...
// --- 以下、追加のサブコンポーネント (崩れないようにロジックを保持) ---
      ...
      <div className="flex-grow overflow-y-auto">
        {allProfiles.filter(p => p.id !== user?.id).map(u => (
          <div key={u.id} className={`flex items-center gap-4 p-4 cursor-pointer transition ${darkMode ? 'hover:bg-gray-900 border-b border-gray-800/50' : 'hover:bg-gray-50 border-b border-gray-100'}`} onClick={() => setDmTarget(u)}>
            <div className="relative">
              <img src={getAvatar(u.username, u.avatar_url)} className="w-14 h-14 rounded-full object-cover shadow-sm" alt="avatar" />
    ...
    <div className={`fixed inset-0 z-[120] flex flex-col animate-in slide-in-from-right duration-300 ${darkMode ? 'bg-black' : 'bg-[#f0f2f5]'}`}>
      <header className={`p-4 flex items-center justify-between border-b sticky top-0 z-10 ${darkMode ? 'bg-black border-gray-800' : 'bg-white'}`}>
        <div className="flex items-center gap-3">
          ...
          <img src={getAvatar(target.username, target.avatar_url)} className="w-10 h-10 rounded-full object-cover" alt="target" />
        ...
        {messages.map(m => (
          <div key={m.id} className={`flex flex-col ${m.sender_id === currentUser.id ? 'items-end' : 'items-start'}`}>
            <div className={`group relative max-w-[80%] flex items-end gap-2 ${m.sender_id === currentUser.id ? 'flex-row-reverse' : 'flex-row'}`}>
              <div className={`p-4 rounded-[1.8rem] text-sm shadow-sm transition-all ${m.sender_id === currentUser.id ? 'bg-blue-600 text-white rounded-tr-none' : (darkMode ? 'bg-gray-800 text-white rounded-tl-none' : 'bg-white text-gray-800 rounded-tl-none')}`}>
      ...
      <div className={`p-4 border-t ${darkMode ? 'bg-black border-gray-800' : 'bg-white'}`}>
        {previewUrl && (
          <div className="mb-2 relative inline-block">
            <img src={previewUrl} className="w-20 h-20 object-cover rounded-xl border-2 border-blue-500" alt="prev" />
        ...
        <div className="divide-y divide-gray-800/10">
          {notifications.map(n => (
            <div key={n.id} className={`p-4 flex gap-4 items-center cursor-pointer transition ${!n.is_read ? (darkMode ? 'bg-blue-900/10' : 'bg-blue-50') : ''}`} onClick={() => { if (n.type === 'follow') openProfile(n.sender_id); else if (n.post_id) setSelectedPost(n.post_id); else openProfile(n.sender_id); }}>
              <img src={getAvatar(n.sender?.username, n.sender?.avatar_url)} className="w-12 h-12 rounded-full object-cover shadow-sm" alt="s" />
      ...
      <div className={`w-full max-w-md rounded-[2.5rem] flex flex-col h-[85vh] overflow-hidden shadow-2xl ${darkMode ? 'bg-black border border-gray-800' : 'bg-white text-black'}`}>
        <div className={`p-4 border-b flex items-center justify-between ${darkMode ? 'border-gray-800' : 'border-gray-100'}`}>
          <div className="flex items-center gap-3" onClick={() => { onClose(); openProfile(post.profiles?.id); }}>
            <img src={getAvatar(post.profiles?.username, post.profiles?.avatar_url)} className="w-8 h-8 rounded-full object-cover" alt="p" />
        ...
        <div className="flex-grow overflow-y-auto">
          <div className="p-5 border-b">
            {post.image_url && <img src={post.image_url} className="w-full rounded-2xl mb-4" alt="c" />}
          ...
          <div className="p-5 space-y-4 pb-10">
            {comments.map(c => (
              <div key={c.id} className="flex gap-3">
                <img src={getAvatar(c.profiles?.username, c.profiles?.avatar_url)} className="w-8 h-8 rounded-full object-cover" alt="av" />
                <div className="flex-grow">
                   ...
                   <p className="text-[10px] text-gray-500 mt-1 ml-2">{formatTime(c.created_at)}</p>
                </div>
              </div>
            ))}
          </div>
...
function ProfileView({ user, activeProfileId, profileInfo, posts, isEditing, setIsEditing, editData, setEditData, handleUpdateProfile, uploading, avatarInputRef, headerInputRef, getAvatar, openProfile, toggleFollow, stats, setShowFollowList, setShowSettings, darkMode, setView, toggleLike, handleShare, setSelectedPost, onDeletePost }) {
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="relative h-44 bg-gray-200">
          <img src={editData.header_url || 'https://images.unsplash.com/photo-1614850523296-d8c1af93d400?w=800&q=80'} className="w-full h-full object-cover" alt="h" />
        ...
        <div className="px-4 space-y-4">
          <div className="flex flex-col items-center gap-2 -mt-12 relative z-10">
            <div className="relative group" onClick={() => avatarInputRef.current.click()}>
              <img src={getAvatar(editData.username, editData.avatar_url)} className="w-24 h-24 rounded-full object-cover border-4 border-blue-500 cursor-pointer bg-white" alt="av" />
          ...
          <div className="space-y-4 pt-4">
            <div className={`p-4 rounded-2xl ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}><label className="text-[10px] font-black text-gray-400 uppercase block mb-1">Display Name</label><input className="w-full bg-transparent outline-none font-bold" value={editData.display_name} onChange={e => setEditData({...editData, display_name: e.target.value})} /></div>
            <div className={`p-4 rounded-2xl ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}><label className="text-[10px] font-black text-gray-400 uppercase block mb-1">Bio</label><textarea className="w-full bg-transparent outline-none font-bold h-24 resize-none" value={editData.bio} onChange={e => setEditData({...editData, bio: e.target.value})} /></div>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <>
      <div className="relative h-44 bg-gray-200">
        <img src={profileInfo.header_url || 'https://images.unsplash.com/photo-1614850523296-d8c1af93d400?w=800&q=80'} className="w-full h-full object-cover" alt="h" />
      ...
      <div className="px-4 relative">
        <div className="flex justify-between items-end -mt-12 mb-4">
          <img src={getAvatar(profileInfo.username, profileInfo.avatar_url)} className={`w-24 h-24 rounded-full border-4 shadow-xl object-cover ${darkMode ? 'border-black bg-black' : 'border-white bg-white'}`} alt="av" />
          <div className="flex gap-2">
            {user?.id === activeProfileId ? (
              ...
              <button onClick={toggleFollow} className={`flex items-center gap-1.5 rounded-full px-5 py-1.5 text-xs font-black uppercase tracking-tighter transition ${stats.isFollowing ? (darkMode ? 'bg-gray-800 text-white' : 'bg-gray-200 text-black') : 'bg-blue-600 text-white'}`}>{stats.isFollowing ? 'Unfollow' : 'Follow'}</button>
            )}
          </div>
        </div>
        <div>
          ...
          <p className="mt-3 text-[15px] font-medium leading-relaxed">{profileInfo.bio || 'Hello!'}</p>
      ...
      <div className={`divide-y mt-8 border-t ${darkMode ? 'border-gray-800 divide-gray-800' : 'border-gray-100 divide-gray-100'}`}>
        {posts.filter(p => p.user_id === activeProfileId).map(post => (<PostCard key={post.id} post={post} openProfile={openProfile} getAvatar={getAvatar} onLike={toggleLike} onShare={handleShare} currentUser={user} darkMode={darkMode} onOpenDetail={() => setSelectedPost(post)} onDelete={onDeletePost} />))}
      </div>
    </>
  );
}

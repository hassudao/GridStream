...
export default function App() {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: `${post.profiles?.display_name}の投稿 - Beta`,
          text: post.content,
  ...
  return (
    <div className={`max-w-md mx-auto min-h-screen pb-20 border-x font-sans relative shadow-2xl transition-colors duration-300 ${darkMode ? 'bg-black text-white border-gray-800' : 'bg-white text-black border-gray-100'}`}>
      ...
      {creatingStory && (
        <StoryCreator file={creatingStory} onClose={() => setCreatingStory(false)} onPublish={async (blob) => {
          setUploading(true); setCreatingStory(false);
          const url = await uploadToCloudinary(blob);
          await supabase.from('stories').insert([{ user_id: user.id, image_url: url }]);
          await notifyFollowers('story'); fetchData(); setUploading(false);
        }} myProfile={myProfile} getAvatar={getAvatar} />
      )}

      {viewingStory && (
        <StoryViewer stories={groupedStories[viewingStory.userId]} initialIndex={viewingStory.index} onClose={() => setViewingStory(null)} userProfile={allProfiles.find(p => p.id === viewingStory.userId)} getAvatar={getAvatar} currentUserId={user.id} onDelete={async (id) => { if(window.confirm("削除しますか？")){ await supabase.from('stories').delete().eq('id', id); setViewingStory(null); fetchData(); }}} />
      )}
          ...
          <div className={`p-4 overflow-x-auto whitespace-nowrap scrollbar-hide flex gap-4 border-b ${darkMode ? 'border-gray-800' : 'border-gray-50'}`}>
            <div className="inline-flex flex-col items-center gap-1 cursor-pointer relative shrink-0">
              <div className="relative">
                <div className={`rounded-full p-[2px] ${groupedStories[user.id] ? 'bg-gradient-to-tr from-yellow-400 via-red-500 to-purple-500' : 'bg-transparent'}`} onClick={() => groupedStories[user.id] ? setViewingStory({ userId: user.id, index: 0 }) : storyInputRef.current.click()}>
              ...
              <input type="file" accept="image/*" ref={storyInputRef} className="hidden" onChange={(e) => { if(e.target.files[0]) setCreatingStory(e.target.files[0]); e.target.value = ''; }} />
            </div>
...
function NotificationCenter({ notifications, getAvatar, openProfile, setSelectedPost, darkMode }) {
  ...
  return (
    <div className="animate-in fade-in pb-10">
      <header className="p-4 border-b font-black text-lg text-center uppercase italic sticky top-0 z-10 bg-inherit/90 backdrop-blur-md flex items-center">
        <ChevronLeft onClick={() => window.history.back() || location.reload()} className="cursor-pointer mr-auto" />
        <span className="absolute left-1/2 -translate-x-1/2">Notifications</span>
      </header>
...
function PostDetailModal({ post, onClose, getAvatar, openProfile, onDelete, onLike, onShare, currentUser, darkMode, refreshPosts, sendNotification }) {
  ...
  async function handleDeleteComment(commentId) {
    ...
    await fetchComments(); refreshPosts();
  }
      ...
      <div className={`w-full max-w-md rounded-[2.5rem] flex flex-col h-[85vh] overflow-hidden shadow-2xl ${darkMode ? 'bg-black border border-gray-800' : 'bg-white text-black'}`}>
        <div className={`p-4 border-b flex items-center justify-between ${darkMode ? 'border-gray-800' : 'border-gray-100'}`}>
          ...
          <div className="flex items-center gap-2">
            {currentUser?.id === post.user_id && <button onClick={() => onDelete(post.id)} className="p-2 text-red-500 hover:bg-red-500/10 rounded-full transition"><Trash2 size={20}/></button>}
              ...
              <div key={c.id} className="flex gap-3 group">
                ...
                <div className={`flex-grow p-3 rounded-2xl relative ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
                  ...
                  {currentUser?.id === c.user_id && (
                    <button onClick={() => handleDeleteComment(c.id)} className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 text-gray-500 hover:text-red-500 transition"><Trash2 size={14}/></button>
                  )}
                </div>
              </div>
            ))}
...
function StoryCreator({ file, onClose, onPublish, myProfile, getAvatar }) {
  ...
  const handlePublish = () => {
    ...
    if (text) {
      ...
      const centerX = canvas.width / 2 + (textStyle.x * (canvas.width / rect.width));
      const centerY = canvas.height / 2 + (textStyle.y * (canvas.height / rect.height));
    ...
    <div className="fixed inset-0 z-[100] bg-black flex flex-col overflow-hidden touch-none" onMouseMove={handleMove} onTouchMove={handleMove} onMouseUp={handleEnd} onTouchEnd={handleEnd}>
      ...
      {!textMode && (
        <div className="absolute bottom-0 w-full p-4 flex justify-between items-center bg-gradient-to-t from-black/90 to-transparent pb-8">
           <div className="flex items-center gap-2"><div className="p-[2px] rounded-full bg-gradient-to-tr from-yellow-400 to-purple-500"><img src={getAvatar(myProfile.username, myProfile.avatar_url)} className="w-8 h-8 rounded-full border-2 border-black" /></div><span className="text-white text-xs font-bold">Your Story</span></div>
...
function StoryViewer({ stories, initialIndex, onClose, userProfile, getAvatar, currentUserId, onDelete }) {
  ...
  const STORY_DURATION = 5000; timerRef = useRef(null); startTimeRef = useRef(null);
  ...
  const startTimer = () => {
    ...
    const animate = () => { if (isPaused) return; const elapsed = Date.now() - startTimeRef.current; const p = (elapsed / STORY_DURATION) * 100; setProgress(p); if (elapsed < STORY_DURATION) timerRef.current = requestAnimationFrame(animate); else nextStory(); };
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black flex items-center justify-center">
      <div className="relative w-full max-w-md h-full flex flex-col" onMouseDown={() => setIsPaused(true)} onMouseUp={() => setIsPaused(false)} onTouchStart={() => setIsPaused(true)} onTouchEnd={() => setIsPaused(false)}>
        <div className="absolute top-0 left-0 right-0 z-20 p-2 flex gap-1 mt-2">{stories.map((_, idx) => (<div key={idx} className="h-0.5 flex-grow bg-white/30 rounded-full overflow-hidden"><div className="h-full bg-white transition-all duration-100 ease-linear shadow-[0_0_5px_white]" style={{ width: idx < currentIndex ? '100%' : idx === currentIndex ? `${progress}%` : '0%' }} /></div>))}</div>
        <div className="absolute top-4 left-0 right-0 z-20 p-3 pt-6 flex justify-between items-center">
          <div className="flex items-center gap-2"><img src={getAvatar(userProfile.username, userProfile.avatar_url)} className="w-8 h-8 rounded-full border border-white/50" /><span className="text-white text-sm font-bold shadow-black drop-shadow-md">{userProfile.display_name}</span></div>
          <div className="flex items-center gap-2">
        ...
        <img src={currentStory.image_url} className="w-full h-full object-contain bg-black" />
      </div>
    </div>
  );
}
    ...
    <article className={`p-4 flex gap-3 transition border-b last:border-0 ${darkMode ? 'border-gray-800' : 'border-gray-50'}`}>
      ...
      <div className="flex-grow min-w-0">
        ...
        <div className="flex justify-between mt-4 text-gray-400 max-w-[200px] items-center">
          ...
          <button onClick={(e) => { e.stopPropagation(); onShare(post); }} className="hover:text-green-500 transition"><Share2 size={18} /></button>
        </div>
      </div>
    </article>
  );
...
function DMScreen({ target, setDmTarget, currentUser, getAvatar, darkMode }) {
  ...
  return (
    <div className={`fixed inset-0 z-[150] flex flex-col animate-in slide-in-from-right duration-300 ${darkMode ? 'bg-black' : 'bg-[#f8f9fa]'}`}>
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-8 bg-white text-black">
      ...
      <form onSubmit={(e) => { e.preventDefault(); if(step < 4) setStep(step+1); else handleAuth(); }} className="w-full max-w-xs space-y-6">
        {step === 1 && (<div><label className="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1">Email Address</label><input type="email" placeholder="example@mail.com" className="w-full bg-gray-100 p-4 rounded-2xl outline-none font-bold focus:ring-2 focus:ring-blue-500 transition" value={email} onChange={(e) => setEmail(e.target.value)} required autoFocus /></div>)}
        {step === 2 && (<div><label className="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1">Create Password</label><input type="password" placeholder="••••••••" className="w-full bg-gray-100 p-4 rounded-2xl outline-none font-bold focus:ring-2 focus:ring-blue-500 transition" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6} autoFocus /></div>)}
        {step === 3 && (<div><label className="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1">Username</label><input type="text" placeholder="unique_id" className="w-full bg-gray-100 p-4 rounded-2xl outline-none font-bold focus:ring-2 focus:ring-blue-500 transition" value={username} onChange={(e) => setUsername(e.target.value.replace(/[^a-zA-Z0-9_]/g, ''))} required minLength={4} maxLength={15} autoFocus /></div>)}
        {step === 4 && (<div><label className="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1">Display Name</label><input type="text" placeholder="Nickname" className="w-full bg-gray-100 p-4 rounded-2xl outline-none font-bold focus:ring-2 focus:ring-blue-500 transition" value={displayName} onChange={(e) => setDisplayName(e.target.value)} required maxLength={50} autoFocus /></div>)}
        <button type="submit" disabled={loading} className="w-full bg-blue-700 text-white font-black py-5 rounded-2xl uppercase tracking-widest text-xs shadow-lg shadow-blue-200 active:scale-95 transition">{loading ? "..." : (step === 4 ? "Complete Sign Up" : "Next")}</button>
      </form>
    </div>
  );
    }

...
import { Camera, MessageCircle, Heart, Share2, Plus, User, Search, Home as HomeIcon } from 'lucide-react';
...
export default function App() {
  ...
  return (
    <div className="max-w-md mx-auto bg-white min-h-screen pb-20 border-x border-gray-100 font-sans text-black relative">
        ...
        <>
          ...
          <div className="p-4 border-b border-gray-100 flex gap-3">
            ...
            <div className="flex-grow">
              ...
              <div className="flex justify-end mt-2"><button onClick={handlePost} className="bg-blue-500 text-white px-5 py-2 rounded-full font-bold text-sm shadow-md">ポスト</button></div>
            </div>
          </div>
      ...
      {/* 2. 検索画面 (Instagram風グリッド) */}
      {view === 'search' && (
        <div className="animate-in fade-in duration-300">
          <div className="p-4 sticky top-0 bg-white z-10">
            <div className="relative">
              <Search className="absolute left-3 top-3 text-gray-400" size={20} />
              <input type="text" placeholder="検索" className="w-full bg-gray-100 border-none rounded-xl py-2.5 pl-10 pr-4 outline-none focus:ring-2 focus:ring-gray-200" />
            </div>
          </div>
            ...
            {[...Array(24)].map((_, i) => (
              <div key={i} className="aspect-square bg-gray-200 relative group overflow-hidden">
                ...
                <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition flex items-center justify-center text-white">
                  <Heart size={16} className="mr-1 fill-white" /> 12
                </div>
              </div>
            ))}
          </div>
...
function LoginScreen({ username, setUsername, setUser }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-6 bg-gray-50 text-black">
      <h1 className="text-4xl font-extrabold mb-8 bg-gradient-to-r from-blue-500 to-purple-500 bg-clip-text text-transparent">GridStream</h1>
      <div className="w-full max-w-sm bg-white p-8 rounded-3xl shadow-xl border border-gray-100">
        <input type="text" className="w-full border-2 border-gray-100 p-4 rounded-2xl mb-4 outline-none" placeholder="ユーザー名" value={username} onChange={(e) => setUsername(e.target.value)} />
        <button onClick={handleSignUp} disabled={loading} className="w-full bg-blue-500 text-white font-bold py-4 rounded-2xl shadow-lg">{loading ? '準備中...' : '新しく始める'}</button>
      </div>
    </div>
  );
                         }

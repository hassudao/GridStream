...
import { Camera, Heart, Share2, Search, Home as HomeIcon, X, User as UserIcon, Grid, List, Image as ImageIcon, Send, ChevronLeft, Zap, Mail, Lock, Settings, Moon, Sun, AlertCircle, Trash2 } from 'lucide-react';
...
export default function App() {
  ...
  async function handleSaveProfile() {
    ...
    if (error) { alert("エラー: ユーザー名重複の可能性があります。"); } else {
  ...
  async function handleDeletePost(postId) {
    ...
    if (error) alert("失敗: " + error.message); else fetchData();
  }
          ...
          {!isEditing && (
            <>
              <div className={`flex border-b mt-6 sticky top-0 z-40 shadow-sm ${darkMode ? 'bg-black/95 border-gray-800' : 'bg-white/95 border-gray-100'}`}>
                <button onClick={() => setProfileTab('list')} className={`flex-grow py-4 flex justify-center items-center gap-2 ${profileTab === 'list' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-300'}`}><List size={20}/><span className="text-[10px] font-black uppercase tracking-tighter">Streams</span></button>
              ...
              <div className={profileTab === 'grid' ? "grid grid-cols-3 gap-[2px]" : `divide-y ${darkMode ? 'divide-gray-800' : 'divide-gray-100'}`}>
                {posts.filter(p => p.user_id === activeProfileId).filter(p => profileTab === 'grid' ? !!p.image_url : true).map(post => (
                  profileTab === 'grid' ? <img key={post.id} src={post.image_url} className="aspect-square w-full h-full object-cover cursor-pointer hover:brightness-90 transition" /> 
                  : <PostCard key={post.id} post={post} openProfile={openProfile} getAvatar={getAvatar} onDelete={handleDeletePost} onLike={toggleLike} currentUser={user} darkMode={darkMode} />
                ))}
              </div>
            </>
    ...
    <div className="animate-in fade-in">
      ...
      <div className="grid grid-cols-3 gap-[2px]">
        {posts.filter(p => p.image_url && (p.content.toLowerCase().includes(searchQuery.toLowerCase()) || p.profiles?.username.toLowerCase().includes(searchQuery.toLowerCase()))).map((post) => (
          <img key={post.id} src={post.image_url} className="aspect-square w-full h-full object-cover cursor-pointer hover:opacity-80 transition" />
        ))}
      </div>
    </div>
  );

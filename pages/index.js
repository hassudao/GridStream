...
export default function App() {
  ...
  async function sendNotification(receiverId, type, postId = null) {
    ...
    await supabase.from('notifications').insert([{
      sender_id: user.id, receiver_id: receiverId, type: type, post_id: postId, is_read: false
    }]);
  }
  ...
  const handleStoryPublish = async (processedImageBlob) => {
    ...
    try {
      ...
      fetchData(); 
    } catch (error) { alert("アップロードに失敗しました"); } finally { setUploading(false); }
  };
        ...
        <div className="animate-in fade-in">
          ...
          <div className={`p-4 overflow-x-auto whitespace-nowrap scrollbar-hide flex gap-4 border-b ${darkMode ? 'border-gray-800' : 'border-gray-50'}`}>
            <div className="inline-flex flex-col items-center gap-1 cursor-pointer relative shrink-0">
              <div className="relative">
                ...
                <div className="absolute bottom-0 right-0 bg-blue-500 rounded-full p-1 border-2 border-black" onClick={(e) => { e.stopPropagation(); storyInputRef.current.click(); }}><Plus size={12} className="text-white" /></div>
              </div>
          ...
          <form onSubmit={handlePost} className={`p-4 border-b ${darkMode ? 'border-gray-800' : 'border-gray-100'}`}>
            ...
            <div className="flex justify-between items-center pl-12 mt-2">
              ...
              <button type="submit" disabled={uploading || !newPost.trim()} className="bg-blue-600 text-white px-6 py-2 rounded-full font-black text-xs uppercase tracking-tighter">{uploading ? '...' : 'Stream'}</button>
            </div>
          </form>
  ...
  return (
    <div className="animate-in fade-in pb-10">
      ...
      {notifications.length === 0 ? (
        <div className="p-20 text-center text-gray-500 font-bold text-xs uppercase">No notifications yet</div>
      ) : (
        ...
        <div className="flex-grow overflow-y-auto scrollbar-hide">
          <div className="p-5 border-b border-gray-100/10">
            ...
            <div className="flex gap-6 text-gray-400">
               <button onClick={() => onLike(post.id, post.is_liked)} className={`flex items-center gap-1.5 ${post.is_liked ? 'text-red-500' : ''}`}><Heart size={20} fill={post.is_liked ? 'currentColor' : 'none'} /><span className="text-xs font-black">{post.like_count || ''}</span></button>
               <button onClick={() => onShare(post)}><Share2 size={20}/></button>
            </div>
          </div>
              ...
              <div key={c.id} className="flex gap-3 items-start group">
                ...
                <div className={`flex-grow p-3 rounded-2xl ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
                  <div className="flex justify-between items-center mb-1">
                    ...
                    {currentUser?.id === c.user_id && <Trash2 size={12} className="text-gray-500 cursor-pointer opacity-0 group-hover:opacity-100 transition" onClick={() => handleDeleteComment(c.id)} />}
                  </div>
...
function StoryCreator({ file, onClose, onPublish, myProfile, getAvatar }) {
  ...
  const handlePublish = () => {
    ...
    if (text) {
      ...
      const rect = document.getElementById('story-preview-container').getBoundingClientRect();
      ...
      {textMode && (
        <div className="absolute inset-0 z-30 bg-black/80 backdrop-blur-sm flex flex-col justify-center items-center animate-in fade-in">
           ...
           <div className="absolute bottom-0 w-full p-4 space-y-4 pb-10 bg-gradient-to-t from-black to-transparent">
             <div className="flex justify-center gap-4 overflow-x-auto pb-2">{FONT_STYLES.map((f, i) => (<button key={f.name} onClick={() => setTextStyle({...textStyle, fontIndex: i})} className={`px-4 py-1 rounded-full text-xs font-bold border ${textStyle.fontIndex === i ? 'bg-white text-black border-white' : 'bg-black/50 text-white border-white/30'}`}>{f.name}</button>))}</div>
             <div className="flex justify-center gap-3 overflow-x-auto px-4">{TEXT_COLORS.map((c, i) => (<button key={c} onClick={() => setTextStyle({...textStyle, colorIndex: i})} className={`w-8 h-8 rounded-full border-2 ${textStyle.colorIndex === i ? 'border-white scale-110' : 'border-transparent'}`} style={{ backgroundColor: c }} />))}</div>
           </div>
        </div>
      )}
      {!textMode && (
        <div className="absolute bottom-0 w-full p-4 flex justify-between items-center bg-gradient-to-t from-black/90 to-transparent pb-8">
           <div className="flex items-center gap-2"><div className="p-[2px] rounded-full bg-gradient-to-tr from-yellow-400 to-purple-500"><img src={getAvatar(myProfile.username, myProfile.avatar_url)} className="w-8 h-8 rounded-full border-2 border-black" /></div><span className="text-white text-xs font-bold uppercase tracking-widest">Your Story</span></div>
           <button onClick={handlePublish} className="bg-white text-black rounded-full p-3 px-6 font-bold flex items-center gap-2 shadow-lg active:scale-95 transition">Share <ChevronLeft className="rotate-180" size={16} /></button>
        </div>
      )}
    </div>
  );
...
function StoryViewer({ stories, initialIndex, onClose, userProfile, getAvatar, currentUserId, onDelete }) {
  ...
  useEffect(() => { if (!currentStory) return; setProgress(0); startTimer(); return () => cancelAnimationFrame(timerRef.current); }, [currentIndex, currentStory]);
    ...
    <div className="fixed inset-0 z-[100] bg-black flex items-center justify-center">
      ...
      <div className="relative w-full max-w-md h-full sm:h-[90vh] sm:rounded-2xl overflow-hidden bg-gray-900 flex flex-col" onMouseDown={handlePointerDown} onMouseUp={handlePointerUp} onTouchStart={handlePointerDown} onTouchEnd={handlePointerUp}>
        ...
        <div className="absolute top-0 left-0 right-0 z-20 p-2 flex gap-1 mt-2">
          {stories.map((_, idx) => (<div key={idx} className="h-0.5 flex-grow bg-white/30 rounded-full overflow-hidden"><div className="h-full bg-white transition-all duration-100 ease-linear shadow-[0_0_5px_rgba(255,255,255,0.8)]" style={{ width: idx < currentIndex ? '100%' : idx === currentIndex ? `${progress}%` : '0%' }} /></div>))}
        </div>
        <div className="absolute top-4 left-0 right-0 z-20 p-3 pt-6 flex justify-between items-center">
          <div className="flex items-center gap-2"><img src={getAvatar(userProfile.username, userProfile.avatar_url)} className="w-8 h-8 rounded-full border border-white/50" /><span className="text-white text-sm font-bold shadow-black drop-shadow-md">{userProfile.display_name}</span><span className="text-white/80 text-[10px] font-bold uppercase">{formatTime(currentStory.created_at)}</span></div>
          <div className="flex items-center gap-2">
  ...
  if (isEditing) {
    return (
      <div className="space-y-6 animate-in slide-in-from-bottom duration-300">
        <header className={`p-4 flex justify-between items-center sticky top-0 z-10 backdrop-blur-md border-b ${darkMode ? 'bg-black/90 border-gray-800' : 'bg-white/90 border-gray-100'}`}>
          ...
          <button onClick={handleUpdateProfile} disabled={uploading} className="bg-blue-600 text-white px-5 py-1.5 rounded-full text-[10px] font-black uppercase">{uploading ? '...' : 'Save'}</button>
        </header>
        ...
        <div className="px-4 space-y-4">
           <div className="flex flex-col items-center gap-2 -mt-12 relative z-10">
             <div className="relative group" onClick={() => avatarInputRef.current.click()}><img src={getAvatar(editData.username, editData.avatar_url)} className="w-24 h-24 rounded-full object-cover border-4 border-blue-500 cursor-pointer bg-white shadow-xl" /><div className="absolute inset-0 flex items-center justify-center bg-black/40 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"><Camera className="text-white"/></div><input type="file" ref={avatarInputRef} className="hidden" accept="image/*" /></div>
           </div>
      ...
      <div className="px-4 relative">
        <div className="flex justify-between items-end -mt-12 mb-4">
          ...
          <div className="flex gap-2">
            {user?.id === activeProfileId ? (
              <button onClick={() => setIsEditing(true)} className={`border rounded-full px-5 py-1.5 text-[10px] font-black uppercase tracking-tighter ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>Edit Profile</button>
            ) : (
              <button onClick={toggleFollow} className={`flex items-center gap-1.5 rounded-full px-5 py-1.5 text-[10px] font-black uppercase tracking-tighter transition ${stats.isFollowing ? (darkMode ? 'bg-gray-800 text-white' : 'bg-gray-200 text-black') : 'bg-blue-600 text-white'}`}>
        ...
        <div>
          ...
          <div className="flex gap-4 mt-4">
            <button onClick={() => setShowFollowList('following')} className="text-sm"><span className="font-black">{stats.following}</span> <span className="text-gray-400 uppercase text-[10px] font-bold">Following</span></button>
            <button onClick={() => setShowFollowList('followers')} className="text-sm"><span className="font-black">{stats.followers}</span> <span className="text-gray-400 uppercase text-[10px] font-bold">Followers</span></button>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 space-y-8">
        <section>
          ...
          <div className="space-y-4">
             ...
             <button onClick={handleUpdateAuth} disabled={updating} className="w-full py-3 bg-blue-600 rounded-xl font-bold text-[10px] uppercase text-white tracking-widest">{updating ? 'Updating...' : 'Update Auth'}</button>
          </div>
        </section>
        <section><h3 className="text-gray-400 text-[10px] font-black uppercase mb-4 tracking-widest">Appearance</h3><button onClick={() => setDarkMode(!darkMode)} className={`w-full flex justify-between items-center p-4 rounded-2xl ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}><span className="text-xs font-black uppercase tracking-widest">Dark Mode</span><div className={`w-10 h-6 rounded-full relative transition-colors ${darkMode ? 'bg-blue-600' : 'bg-gray-300'}`}><div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${darkMode ? 'right-1' : 'left-1'}`} /></div></button></section>
        <section className="pt-4 border-t border-gray-100/10"><button onClick={handleLogout} className="w-full p-4 rounded-2xl bg-red-500/10 text-red-500 font-black uppercase text-[10px] tracking-widest flex items-center justify-center gap-2"><LogOut size={16}/> Logout</button></section>
      </div>
    </div>
  );
}
    ...
    <article className={`p-4 flex gap-3 transition border-b last:border-0 ${darkMode ? 'border-gray-800' : 'border-gray-50'}`}>
      ...
      <div className="flex-grow min-w-0">
        <div className="flex justify-between items-start mb-1">
          ...
          <span className="text-[10px] text-gray-400 font-bold whitespace-nowrap pt-1 uppercase">{formatTime(post.created_at)}</span>
        </div>
        ...
        <div className="flex justify-between mt-4 text-gray-400 max-w-[220px] items-center">
...
function MessagesList({ allProfiles, user, setDmTarget, getAvatar, openProfile, darkMode }) {
  return (
    <div className="animate-in fade-in">
      ...
      <div className="p-2">{allProfiles.filter(p => p.id !== user?.id).map(u => (<div key={u.id} className="flex items-center gap-4 p-4 rounded-2xl cursor-pointer hover:bg-gray-50/10 transition" onClick={() => setDmTarget(u)}><img src={getAvatar(u.username, u.avatar_url)} className="w-14 h-14 rounded-full object-cover shadow-md" onClick={(e) => { e.stopPropagation(); openProfile(u.id); }} /><div className="flex-grow border-b pb-2 border-gray-100/10"><p className="font-bold text-sm">{u.display_name}</p><p className="text-[10px] text-blue-500 font-black mt-1 italic uppercase tracking-tighter">Tap to Chat</p></div></div>))}</div>
    </div>
  );
}
...
function DMScreen({ target, setDmTarget, currentUser, getAvatar, darkMode }) {
  ...
  useEffect(() => {
    fetchMessages();
    const channel = supabase.channel(`chat:${target.id}`).on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages' }, (p) => { if ((p.new.sender_id === currentUser.id && p.new.receiver_id === target.id) || (p.new.sender_id === target.id && p.new.receiver_id === currentUser.id)) setMessages(prev => [...prev, p.new]); }).subscribe();
    return () => supabase.removeChannel(channel);
  }, [target]);
  ...
  return (
    <div className={`fixed inset-0 z-50 flex flex-col animate-in slide-in-from-right duration-300 ${darkMode ? 'bg-black' : 'bg-[#f8f9fa]'}`}>
      <header className={`p-4 flex items-center gap-3 border-b sticky top-0 z-10 ${darkMode ? 'bg-gray-900 border-gray-800' : 'bg-white'}`}><ChevronLeft onClick={() => setDmTarget(null)} className="cursor-pointer" /><img src={getAvatar(target.username, target.avatar_url)} className="w-10 h-10 rounded-full object-cover" /><div><p className="font-black text-sm">{target.display_name}</p><p className="text-[10px] text-gray-400 font-bold">@{target.username}</p></div></header>
      <div className="flex-grow overflow-y-auto p-4 space-y-4">{messages.map(m => (<div key={m.id} className={`flex ${m.sender_id === currentUser.id ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[80%] p-4 rounded-[1.5rem] text-sm ${m.sender_id === currentUser.id ? 'bg-blue-600 text-white rounded-tr-none shadow-lg' : (darkMode ? 'bg-gray-800 text-white rounded-tl-none' : 'bg-white text-gray-800 rounded-tl-none shadow-sm')}`}>{m.text}</div></div>))}<div ref={scrollRef} /></div>
      <form onSubmit={sendMsg} className={`p-4 border-t flex gap-2 ${darkMode ? 'bg-gray-900 border-gray-800' : 'bg-white'}`}><input type="text" className={`flex-grow p-4 rounded-2xl text-sm outline-none font-medium ${darkMode ? 'bg-black text-white' : 'bg-gray-50'}`} placeholder="Aa" value={text} onChange={(e) => setText(e.target.value)} /><button type="submit" className="bg-blue-600 text-white p-4 rounded-2xl shadow-lg active:scale-95 transition"><Send size={18}/></button></form>
    </div>
  );
}
...
function FollowListModal({ type, userId, onClose, openProfile, getAvatar, darkMode }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/60 backdrop-blur-sm flex items-end justify-center">
      <div className={`w-full max-w-md rounded-t-[2.5rem] max-h-[80vh] flex flex-col animate-in slide-in-from-bottom duration-300 ${darkMode ? 'bg-black text-white border-t border-gray-800' : 'bg-white'}`}>
        <div className="p-6 border-b border-gray-100/10 flex justify-between items-center"><h3 className="font-black uppercase tracking-widest text-xs">{type}</h3><X onClick={onClose} className="cursor-pointer" /></div>
        <div className="overflow-y-auto p-4 space-y-2">{list.map(u => (<div key={u.id} className="flex items-center gap-4 cursor-pointer p-3 rounded-2xl hover:bg-gray-50/10" onClick={() => { onClose(); openProfile(u.id); }}><img src={getAvatar(u.username, u.avatar_url)} className="w-12 h-12 rounded-full object-cover" /><div className="flex-grow"><p className="font-black text-sm">{u.display_name}</p><p className="text-gray-400 text-[10px] font-bold">@{u.username}</p></div></div>))}</div>
      </div>
    </div>
  );
}
...
function AuthScreen({ fetchData }) {
  const [isLogin, setIsLogin] = useState(true); const [step, setStep] = useState(1);
  ...
  async function handleAuth(e) {
    ...
    try {
      if (isLogin) { const { error } = await supabase.auth.signInWithPassword({ email, password }); if (error) throw error; }
        ...
        if (data?.user) await supabase.from('profiles').upsert([{ id: data.user.id, username: username.toLowerCase(), display_name: displayName }]);
      }
      fetchData();
    } catch (err) { alert(err.message); } finally { setLoading(false); }
  }
  const nextStep = (e) => { e.preventDefault(); if (step === 1 && email) setStep(2); else if (step === 2 && password.length >= 6) setStep(3); else if (step === 3 && username.length >= 4) setStep(4); };
  if (isLogin) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-8 bg-white text-black">
        ...
        <form onSubmit={handleAuth} className="w-full max-w-xs space-y-4">
          ...
          <button type="submit" disabled={loading} className="w-full bg-blue-700 text-white font-black py-5 rounded-2xl uppercase tracking-widest text-[10px] shadow-xl shadow-blue-100">{loading ? "..." : "Login"}</button>
        </form>
        <button onClick={() => { setIsLogin(false); setStep(1); }} className="mt-8 text-[10px] font-black text-gray-400 uppercase tracking-widest">Create Account</button>
      </div>
    );
  }
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-8 bg-white text-black">
      <div className="w-full max-w-xs flex items-center mb-8"><button onClick={() => step > 1 ? setStep(step-1) : setIsLogin(true)} className="p-2 -ml-2 hover:bg-gray-100 rounded-full transition"><ChevronLeft size={24} /></button><div className="flex-grow text-center mr-6"><span className="text-[10px] font-black text-blue-500 uppercase tracking-widest">Step {step} of 4</span></div></div>
      ...
      <form onSubmit={step === 4 ? handleAuth : nextStep} className="w-full max-w-xs space-y-6">
        {step === 1 && (<div className="animate-in slide-in-from-right"><label className="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1">Email</label><input type="email" placeholder="mail@example.com" className="w-full bg-gray-100 p-4 rounded-2xl outline-none font-bold" value={email} onChange={(e) => setEmail(e.target.value)} required autoFocus /></div>)}
        {step === 2 && (<div className="animate-in slide-in-from-right"><label className="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1">Password</label><input type="password" placeholder="••••••••" className="w-full bg-gray-100 p-4 rounded-2xl outline-none font-bold" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6} autoFocus /></div>)}
        {step === 3 && (<div className="animate-in slide-in-from-right"><label className="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1">Username</label><input type="text" placeholder="unique_id" className="w-full bg-gray-100 p-4 rounded-2xl outline-none font-bold" value={username} onChange={(e) => setUsername(e.target.value.replace(/[^a-zA-Z0-9_]/g, ''))} required minLength={4} autoFocus /></div>)}
        {step === 4 && (<div className="animate-in slide-in-from-right"><label className="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1">Display Name</label><input type="text" placeholder="Nickname" className="w-full bg-gray-100 p-4 rounded-2xl outline-none font-bold" value={displayName} onChange={(e) => setDisplayName(e.target.value)} required autoFocus /></div>)}
        <button type="submit" disabled={loading} className="w-full bg-blue-700 text-white font-black py-5 rounded-2xl uppercase tracking-widest text-[10px] shadow-lg active:scale-95 transition">{loading ? "..." : (step === 4 ? "Complete" : "Next")}</button>
      </form>
    </div>
  );
                                                }

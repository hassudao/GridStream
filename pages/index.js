...
export default function App() {
  ...
  const [unreadDms, setUnreadDms] = useState(0); // DM未読用
  ...
  return (
    <div className={`max-w-md mx-auto min-h-screen pb-20 border-x font-sans relative shadow-2xl transition-colors duration-300 ${darkMode ? 'bg-black text-white border-gray-800' : 'bg-white text-black border-gray-100'}`}>
      ...
      {dmTarget && <DMScreen target={dmTarget} setDmTarget={setDmTarget} currentUser={user} getAvatar={getAvatar} darkMode={darkMode} uploadToCloudinary={uploadToCloudinary} onMessageAction={fetchUnreadDmCount} />}
      ...
      {view === 'search' && <SearchView posts={posts} allProfiles={allProfiles} openProfile={openProfile} searchQuery={searchQuery} setSearchQuery={setSearchQuery} setSelectedPost={setSelectedPost} darkMode={darkMode} />}
      ...
      <nav className={`fixed bottom-0 max-w-md w-full border-t flex justify-around py-4 z-40 shadow-sm ${darkMode ? 'bg-black/95 border-gray-800 text-gray-600' : 'bg-white/95 border-gray-100 text-gray-300'}`}>
        ...
        <div className="relative">
          ...
          {unreadDms > 0 && <span className="absolute -top-1 -right-1 bg-blue-600 text-white text-[10px] w-4 h-4 flex items-center justify-center rounded-full font-bold border-2 border-black animate-pulse">{unreadDms}</span>}
        </div>
...
function MessagesList({ allProfiles, user, setDmTarget, getAvatar, openProfile, darkMode }) {
  ...
  useEffect(() => {
    fetchLastMessages();
    const sub = supabase.channel('messages_list')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'messages' }, () => fetchLastMessages())
      .subscribe();
          ...
          <div key={u.id} className={`flex items-center gap-4 p-4 cursor-pointer transition ${darkMode ? 'hover:bg-gray-900 border-b border-gray-800/50' : 'hover:bg-gray-50 border-b border-gray-100'}`} onClick={() => setDmTarget(u)}>
            ...
            <div className="flex-grow pb-1">
              ...
              <div className="flex items-center justify-between">
                <p className={`text-xs truncate max-w-[180px] ${lastMessages[u.id]?.sender_id !== user.id && !lastMessages[u.id]?.is_read ? 'text-white font-bold' : 'text-gray-400'}`}>
                ...
                {lastMessages[u.id] && lastMessages[u.id].sender_id !== user.id && !lastMessages[u.id].is_read && (
                  <div className="w-2.5 h-2.5 bg-blue-600 rounded-full shadow-[0_0_8px_rgba(37,99,235,0.8)]"></div>
                )}
              </div>
            </div>
          </div>
...
function DMScreen({ target, setDmTarget, currentUser, getAvatar, darkMode, uploadToCloudinary, onMessageAction }) {
  ...
  async function markAsRead() {
    ...
    if (!error) onMessageAction(); // メインの未読数を更新
  }
...
function SearchView({ posts, allProfiles, openProfile, searchQuery, setSearchQuery, setSelectedPost, darkMode }) {
    ...
    <div className="animate-in fade-in pb-20">
      ...
      {!searchQuery.trim() ? (
        ...
        <div className="grid grid-cols-3 gap-[2px]">
          {images.map((post) => (
            <img 
              key={post.id} 
              src={post.image_url} 
              className="aspect-square w-full h-full object-cover cursor-pointer hover:opacity-80 transition" 
        ...
        <div className="space-y-6 p-4">
              ...
              <div className="space-y-3">
                {filteredUsers.slice(0, 5).map(u => (
                  <div key={u.id} className="flex items-center gap-3 cursor-pointer" onClick={() => openProfile(u.id)}>
                    <img src={u.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${u.username}`} className="w-10 h-10 rounded-full object-cover" />
                    <div>
                      <p className="font-black text-sm">{u.display_name}</p>
          ...
          {/* 画像検索結果 */}
          {filteredPosts.filter(p => p.image_url).length > 0 && (
            <section>
              <h3 className="text-[10px] font-black text-gray-500 uppercase mb-3 tracking-widest flex items-center gap-2">
                <ImageIcon size={12}/> Images
              </h3>
              <div className="grid grid-cols-3 gap-1">
                {filteredPosts.filter(p => p.image_url).map(p => (
                  <img key={p.id} src={p.image_url} className="aspect-square object-cover rounded-lg cursor-pointer" onClick={() => setSelectedPost(p)} />
                ))}
              </div>
            </section>
          )}
          ...
          {filteredPosts.length > 0 && (
            <section>
              ...
              <div className="space-y-4">
                {filteredPosts.map(p => (
                  <div key={p.id} className={`p-4 rounded-2xl border ${darkMode ? 'border-gray-800 bg-gray-900/50' : 'border-gray-100 bg-gray-50'}`} onClick={() => setSelectedPost(p)}>
                    <div className="flex items-center gap-2 mb-2">
                      <img src={p.profiles?.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${p.profiles?.username}`} className="w-5 h-5 rounded-full" />
                      <span className="text-[10px] font-black">@{p.profiles?.username}</span>
                    </div>

...
export default function App() {
  ...
  const toggleFollow = async () => {
    ...
    openProfile(activeProfileId); // 状態リフレッシュ
  };
    ...
    <div className="max-w-md mx-auto bg-white min-h-screen pb-20 border-x border-gray-100 font-sans text-black relative shadow-2xl overflow-x-hidden">
      ...
      {view === 'profile' && profileInfo && (
        <div className="animate-in fade-in pb-10">
          <div className={`h-32 relative overflow-hidden ${!profileInfo.header_url && 'bg-gradient-to-br from-blue-700 via-indigo-600 to-cyan-500'}`}>
            {profileInfo.header_url && <img src={profileInfo.header_url} className="w-full h-full object-cover" />}
            <button onClick={() => setView('home')} className="absolute top-4 left-4 bg-black/20 backdrop-blur-md p-2 rounded-full text-white"><ChevronLeft size={20}/></button>
            {user.id === activeProfileId && (
              <button onClick={handleLogout} className="absolute top-4 right-4 bg-white/20 backdrop-blur-md p-2 rounded-full text-white hover:bg-red-500 transition"><LogOut size={18}/></button>
            )}
          </div>
          
          <div className="px-4 relative">
            <div className="absolute -top-12 left-4">
              <img src={getAvatar(profileInfo.username, profileInfo.avatar_url)} className="w-24 h-24 rounded-full border-4 border-white bg-white shadow-xl object-cover" />
            </div>
            ...
            <div className="mt-2 space-y-3">
              ...
              <div className="flex gap-6 pt-1">
                ...
                <button onClick={() => setShowFollowList('followers')} className="hover:opacity-60 transition flex gap-1.5 items-center">
                  <span className="font-black text-lg">{stats.followers}</span><span className="text-gray-400 text-[10px] font-black uppercase tracking-widest">Followers</span>
                </button>
              </div>
            </div>
          </div>
          ...
          <div className={profileTab === 'grid' ? "grid grid-cols-3 gap-[2px]" : "divide-y divide-gray-100"}>
            {posts.filter(p => p.user_id === activeProfileId).map(post => 
              profileTab === 'grid' ? (
                post.image_url && <img key={post.id} src={post.image_url} className="aspect-square object-cover cursor-pointer hover:brightness-90 transition" onClick={() => setSelectedPost(post)} />
              ) : (
...
function MessagesList({ allProfiles, user, setDmTarget, getAvatar, openProfile }) {
  return (
    <div className="animate-in fade-in">
      <header className="p-4 border-b border-gray-100 font-black text-lg text-center tracking-tighter uppercase sticky top-0 bg-white/95 z-10 italic">Stream Chat</header>
      <div className="p-2">
        {allProfiles.filter(p => p.id !== user.id).map(u => (
          <div key={u.id} className="flex items-center gap-4 p-4 hover:bg-gray-50 rounded-2xl cursor-pointer transition">
            <img src={getAvatar(u.username, u.avatar_url)} className="w-14 h-14 rounded-full bg-gray-100 object-cover" onClick={() => openProfile(u.id)} />
            <div className="flex-grow border-b border-gray-50 pb-2" onClick={() => setDmTarget(u)}>
              <p className="font-bold text-sm">{u.display_name} <span className="text-gray-400 font-bold text-xs ml-1">@{u.username}</span></p>
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#f8f9fa] flex flex-col animate-in slide-in-from-right duration-300">
      <header className="bg-white p-4 flex items-center gap-3 border-b border-gray-100 sticky top-0">
        ...
        <div>
          <p className="font-black text-sm leading-tight">{target.display_name}</p>
          <p className="text-[10px] text-gray-400 font-bold">@{target.username}</p>
        </div>
      </header>

  ...
  return (
    <div className={`max-w-md mx-auto min-h-screen pb-20 border-x font-sans relative shadow-2xl transition-colors duration-300 ${darkMode ? 'bg-black text-white border-gray-800' : 'bg-white text-black border-gray-100'}`}>
        ...
        <div className="animate-in fade-in">
          ...
          <form onSubmit={handlePost} className={`p-4 border-b ${darkMode ? 'border-gray-800' : 'border-gray-100'}`}>
            ...
            <div className="flex justify-between items-center pl-12 mt-2">
              <label className="cursor-pointer text-blue-500 p-2"><ImageIcon size={22}/><input type="file" accept="image/*" ref={fileInputRef} className="hidden" /></label>
              <button type="submit" disabled={uploading || !newPost.trim()} className="bg-blue-600 text-white px-6 py-2 rounded-full font-black text-xs uppercase">{uploading ? '...' : 'Stream'}</button>
            </div>
          </form>
      ...
      {view === 'profile' && profileInfo && (
        <div className="animate-in fade-in pb-10">
          <div className={`h-32 relative bg-gray-200 ${!profileInfo.header_url && 'bg-gradient-to-br from-blue-700 to-cyan-500'}`}>
            ...
            {isEditing && <label className="absolute inset-0 bg-black/40 flex items-center justify-center cursor-pointer text-white"><Camera size={24} /><input type="file" ref={headerInputRef} className="hidden" onChange={(e) => setEditData({...editData, header_url: URL.createObjectURL(e.target.files[0])})} /></label>}
            ...
            {!isEditing && user.id === activeProfileId && <button onClick={() => setShowSettings(true)} className="absolute top-4 right-4 bg-black/30 backdrop-blur-md p-2 rounded-full text-white"><Settings size={20}/></button>}
          </div>
          
          <div className="px-4 relative">
            <div className="absolute -top-12 left-4">
              <div className="relative">
                <img src={isEditing ? getAvatar(editData.username, editData.avatar_url) : getAvatar(profileInfo.username, profileInfo.avatar_url)} className={`w-24 h-24 rounded-full border-4 object-cover ${darkMode ? 'border-black bg-black' : 'border-white bg-white'}`} />
                {isEditing && <label className="absolute inset-0 bg-black/40 rounded-full flex items-center justify-center cursor-pointer text-white"><Camera size={20} /><input type="file" ref={avatarInputRef} className="hidden" onChange={(e) => setEditData({...editData, avatar_url: URL.createObjectURL(e.target.files[0])})} /></label>}
              </div>
            </div>
            <div className="flex justify-end py-3">
              {user.id === activeProfileId ? (
                ...
                <button onClick={toggleFollow} className={`rounded-full px-6 py-1.5 text-xs font-black uppercase transition ${stats.isFollowing ? 'bg-gray-100 text-black border' : 'bg-blue-600 text-white'}`}>{stats.isFollowing ? 'Following' : 'Follow'}</button>
              )}
            </div>
            <div className="mt-4 space-y-2">
              ...
              <div className="flex gap-4 pt-2">
                <button onClick={() => setShowFollowList('following')} className="text-sm font-black">{stats.following} <span className="text-gray-400 font-bold uppercase text-[10px]">Following</span></button>
                <button onClick={() => setShowFollowList('followers')} className="text-sm font-black">{stats.followers} <span className="text-gray-400 font-bold uppercase text-[10px]">Followers</span></button>
              </div>
            </div>
          </div>
          
          <div className={`flex border-b mt-6 ${darkMode ? 'border-gray-800' : 'border-gray-100'}`}>
            <button onClick={() => setProfileTab('list')} className={`flex-grow py-4 text-[10px] font-black uppercase ${profileTab === 'list' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-400'}`}>Threads</button>
            <button onClick={() => setProfileTab('grid')} className={`flex-grow py-4 text-[10px] font-black uppercase ${profileTab === 'grid' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-400'}`}>Media</button>
          </div>
      ...
      <nav className={`fixed bottom-0 max-w-md w-full border-t flex justify-around py-4 z-40 ${darkMode ? 'bg-black/95 border-gray-800 text-gray-600' : 'bg-white/95 border-gray-100 text-gray-300'}`}>
        ...
        <Search onClick={() => setView('search')} className={view === 'search' ? (darkMode ? 'text-white' : 'text-black') : ''} />
        <MessageCircle onClick={() => setView('messages')} className={view === 'messages' ? (darkMode ? 'text-white' : 'text-black') : ''} />
        <UserIcon onClick={() => openProfile(user.id)} className={view === 'profile' && activeProfileId === user.id ? (darkMode ? 'text-white' : 'text-black') : ''} />
      </nav>
    </div>
  );
}
...
function PostCard({ post, openProfile, getAvatar, onDelete, onLike, onSendComment, onDeleteComment, currentUser, darkMode }) {
  ...
  return (
    <div className={`p-4 flex flex-col gap-3 border-b last:border-0 ${darkMode ? 'border-gray-800' : 'border-gray-50'}`}>
      <div className="flex gap-3">
              ...
              <div key={comment.id} className="flex gap-2 text-sm group">
                ...
                <div className="flex-grow">
                  <div className="flex items-center gap-2">
                    ...
                    {currentUser.id === comment.user_id && <button onClick={() => onDeleteComment(comment.id)} className="opacity-0 group-hover:opacity-100 text-red-400 transition"><Trash2 size={12} /></button>}
                  </div>
                  <p className="font-medium text-[13px] opacity-90">{comment.content}</p>
                </div>
              </div>
            ))}
          </div>
          <form onSubmit={(e) => { e.preventDefault(); onSendComment(post.id, commentText); setCommentText(''); }} className="flex gap-2 items-center">
            <input type="text" placeholder="Stream your reply..." className={`flex-grow text-xs font-bold p-3 rounded-xl outline-none ${darkMode ? 'bg-gray-900 text-white' : 'bg-gray-50'}`} value={commentText} onChange={(e) => setCommentText(e.target.value)} />
...
function DMScreen({ target, setDmTarget, currentUser, getAvatar, darkMode }) {
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex flex-col animate-in slide-in-from-right duration-300 ${darkMode ? 'bg-black text-white' : 'bg-white text-black'}`}>
      <header className="p-4 flex items-center gap-3 border-b sticky top-0 bg-inherit z-10">
      ...
      <div className="flex-grow overflow-y-auto p-4 space-y-4">
        {messages.map(m => (
          <div key={m.id} className={`flex ${m.sender_id === currentUser.id ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[80%] p-4 rounded-[1.5rem] text-sm ${m.sender_id === currentUser.id ? 'bg-blue-600 text-white rounded-tr-none' : (darkMode ? 'bg-gray-800 text-white rounded-tl-none' : 'bg-gray-100 text-black rounded-tl-none')}`}>{m.text}</div>
          </div>
        ))}
        <div ref={scrollRef} />
      </div>
      <form onSubmit={sendMsg} className="p-4 border-t flex gap-2">
        <input type="text" className={`flex-grow p-4 rounded-2xl text-sm outline-none ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`} placeholder="Aa" value={text} onChange={(e) => setText(e.target.value)} />
    ...
    <div className={`fixed inset-0 z-[100] animate-in slide-in-from-bottom duration-300 ${darkMode ? 'bg-black text-white' : 'bg-white text-black'}`}>
      ...
      <div className="p-4 space-y-6">
        <button onClick={() => setDarkMode(!darkMode)} className={`w-full flex justify-between items-center p-4 rounded-2xl ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
          <div className="flex items-center gap-3">{darkMode ? <Moon size={18}/> : <Sun size={18}/>}<span className="text-sm font-bold">Dark Mode</span></div>
        ...
        <button onClick={handleLogout} className="w-full p-4 rounded-2xl bg-red-50 text-red-500 font-black uppercase text-xs tracking-widest">Logout</button>
      </div>
    </div>
  );
}
...
function FollowListModal({ type, userId, onClose, openProfile, getAvatar, darkMode }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/60 backdrop-blur-sm flex items-end justify-center">
      <div className={`w-full max-w-md rounded-t-[2.5rem] max-h-[80vh] flex flex-col ${darkMode ? 'bg-black' : 'bg-white'}`}>
        <div className="p-6 border-b flex justify-between items-center rounded-t-[2.5rem]"><h3 className="font-black text-lg uppercase text-blue-600">{type}</h3><X onClick={onClose} className="cursor-pointer" /></div>
        <div className="overflow-y-auto p-4 space-y-2">
          {list.map(u => (
            <div key={u.id} className="flex items-center gap-4 cursor-pointer p-3 rounded-2xl hover:bg-gray-50/10" onClick={() => { openProfile(u.id); onClose(); }}>
